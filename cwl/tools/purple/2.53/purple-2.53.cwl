...
requirements:
  ResourceRequirement:
    ...
    ramMin: 16000
  DockerRequirement:
    dockerPull: quay.io/biocontainers/hmftools-purple:2.53--0
  NetworkAccess:
    networkAccess: true
...
inputs:
  ...
  germline_vcf:
    type: File?
    doc: |
      Optional location of germline variants vcf. Sample names must match reference parameter. GZ files supported.
    inputBinding:
      prefix: "-germline_vcf"
    secondaryFiles:
      - ".tbi"
  ...
  germline_hotspots:
    type: File?
    doc: |
      VCF of germline hotspot locations. Mandatory if driver catalog enabled and germline variants supplied.
    inputBinding:
      prefix: "-germline_hotspots"
  somatic_hotspots:
    type: boolean?
    doc: |
